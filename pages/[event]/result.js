...
export default function Result() {
  ...
  useEffect(() => {
        ...
        const rawPhoto = sessionStorage.getItem("rawPhoto");
        if (!name || !tId || !rawPhoto || !event) {
        ...
        setShareSupported(typeof navigator !== "undefined" && !!navigator.share);
                ...
                const draw = async () => {
                        ...
                        const img = new Image();
                        img.onload = () => renderCanvas(img, template, name);
                        img.src = rawPhoto;
                };
        draw();
  ...
  function renderCanvas(img, template, name) {
      ...
      const isPolaroid = template.style === "polaroid";
        const padding = isPolaroid ? 60 : 90;
        const photoW = CANVAS_W - padding * 2;
        const photoH = photoW * 1.05;
        const photoX = padding;
        const photoY = isPolaroid ? padding : 100;
      ...
      if (!isPolaroid) {
              drawCorners(ctx, photoX, photoY, photoW, photoH, template.accent);
      }

      ctx.fillStyle = template.textColor;
        ctx.textAlign = "center";
        ctx.font = "90px 'Great Vibes', cursive";
        const nameY = photoY + photoH + (isPolaroid ? 100 : 110);
  ...
  async function persistGuest(dataUrl) {
        ...
        try {
                const res = await fetch("/api/guests", {
                          ...
                          body: JSON.stringify({ name: guestName, templateId, photoDataUrl: dataUrl }),
                });
  ...
  function handleRetake() {
        sessionStorage.removeItem("rawPhoto");
